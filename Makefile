...
.PHONY: help dev-infra-up dev-migrate dev-frontend dev-backend dev-script-runtime dev-media-processing-runtime dev test-go test-go-integration test
...
help:
	@printf '%s\n' \
		...
		'make dev                同时启动前端、Go backend 和两个私有运行时' \
...
dev-script-runtime:
	$(DEV_ENV) sh -c 'SCRIPT_RUNTIME_TOKEN="$$GO_SCRIPT_RUNTIME_TOKEN" exec node services/api-upstream-script-runtime/server.mjs'

dev-media-processing-runtime:
	$(DEV_ENV) sh -c 'MEDIA_PROCESSING_TOKEN="$$GO_MEDIA_PROCESSING_TOKEN" exec node services/media-processing-runtime/server.mjs'
...
dev: dev-migrate
	@trap 'kill 0' INT TERM EXIT; \
		...
		$(MAKE) dev-media-processing-runtime & \
		wait
