...
- 公开 `/api-docs` 与控制台镜像共用同一数据源，必须同步展示模型、积分、图片与视频八个
  ...
  比例、分辨率、输入图互斥、声音能力与持久任务轮询契约。接口目录与正文按接入基础、
  生成图片、生成视频三个模块一一编排，任务查询归入对应媒体模块；电梯模块标题只控制
  展开折叠，公开页与控制台必须复用同一响应式电梯。
...
- 管理端“模型配置”以单条目保存价格、运行启用开关、展示开关、简介和封面；公开目录由
...
- `enabled` 缺失时按 `true` 兼容旧配置；显式停用后，模型不得进入 `/v1/models`、
  `video.listCapabilities`、图片/视频创作目录、`/models` 或首页，也不得创建新的图片或
  视频生成。`visible` 仍只控制已启用模型的 `/models` 与首页展示，不参与套餐能力、
  成员配置或价格计算。
...
- 管理员运营总览使用独立 `operations.*` UOL，不扩展支持用户筛选和最多 30 天范围的
  既有生成看板契约。
- 新增、网页活跃、创作留存和支付生命周期只从正式生产 epoch 之后累积；生产发布在新
  Web 启动后通过 `operations.ensureCurrentEpoch` 自动确保 epoch，空表时取
  `APP_TIME_ZONE` 当前自然日，已有值时原样跳过。调用方不能传日期；迁移与人工命令
  均不得直接写入 epoch，错误起点只能通过受审批的新迁移修复。
