...
> Seedance 2.0 脱敏 HAR，以及 2026-07-28 的 Adobe Firefly Kling 3.0 Omni 与
> Runway Gen-4.5 脱敏请求校正网页真实协议。仅记录接口契约（端点/字段/枚举），不保存
...
`seedance2*`、`seedance2-fast*` 裸模型，内部统一归一化后按同一目录派发。
...
与 Runway Gen-4.5 使用 `https://firefly.adobe.com/` 与公开网页 API Key
`clio-playground-web`。Seedance 提交明确携带 `x-arp-session-id` 和
...
`false`，Kling 3.0 默认 `true`；调用方可以逐次覆盖。Runway Gen-4.5 不支持声音，
其提交体不发送 `generateAudio`。其他目录模型当前不声明音频能力，传 `true` 会在
统一接口层拒绝，传 `false` 可兼容统一客户端。
