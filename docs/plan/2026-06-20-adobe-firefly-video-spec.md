...
> Runway Gen-4.5 与 Ray 3.14 脱敏请求校正网页真实协议。仅记录接口契约
> （端点/字段/枚举），不保存
...
`ray314*`、`seedance2*`、`seedance2-fast*` 裸模型，内部统一归一化后按同一目录派发。
...
Runway Gen-4.5 与 Ray 3.14 使用 `https://firefly.adobe.com/` 与公开网页 API Key
...
不支持声音，其提交体不发送 `generateAudio`。其他目录模型当前不声明音频能力，传
`true` 会在统一接口层拒绝，传 `false` 可兼容统一客户端。
