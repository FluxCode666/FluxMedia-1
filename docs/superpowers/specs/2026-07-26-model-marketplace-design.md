...
当前管理后台已有独立“模型计费”页签，统一管理图像模型四档固定积分与视频模型族每秒积分；官网首页也已有基于运行时事实构建的公开模型目录，但缺少完整模型广场、模型展示配置、封面和详情。
...
每张视频卡提供一个由代码稳定选出的默认且可调用完整 ID，例如：

```text
firefly-veo31-4s-16x9-1080p
```

复制按钮复制该完整 ID。“立即使用”进入创作页后，用户仍可修改时长、比例和分辨率。
