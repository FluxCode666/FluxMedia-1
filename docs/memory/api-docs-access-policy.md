...
公开页的接口区使用响应式滚动电梯：桌面端为 15.5rem 的圆角目录卡，按接入基础、图片
API、视频 API、任务查询四个模块展示；正文使用相同模块和稳定锚点。窄屏退化为粘性
横向导航，滚动仅发生在目录内部，不得扩大页面宽度。模块标题和端点活动状态随滚动位置
更新，并通过文字、背景和 `aria-current` 同时表达。
