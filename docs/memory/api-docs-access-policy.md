...
接入页 Base URL、全部 curl/响应示例、管理员外部 API 文档和 API 密钥控制台统一读取
当前请求的 `X-Forwarded-Host`/`Host` 与 `X-Forwarded-Proto`，经 HTTP(S) origin 校验后
按当前访问域名展示；请求头缺失或非法时才回退 `getSiteBaseUrl()`。禁止在任一入口单独
硬编码生产域名，也禁止修改模块级共享文档对象，避免多域名并发请求互相串值。

公开页的接口区使用响应式滚动电梯：桌面端为 15.5rem 的圆角目录卡，按接入基础、生成
图片、生成视频三个模块展示；图片与视频任务查询分别归入对应生成模块。正文使用相同模块
和稳定锚点。窄屏退化为粘性横向导航，滚动仅发生在目录内部，不得扩大页面宽度。模块
标题只控制展开折叠，不改变页面位置；端点活动状态随滚动位置更新，并通过文字、背景和
`aria-current` 同时表达。
控制台镜像复用相同电梯；控制台内容容器在该路由保持 `overflow: visible`，避免横向滚动
祖先破坏桌面端 `sticky`，具体代码块和长字段仍由组件自身承担横向滚动。
独立接入页内容宽度上限为 `max-w-7xl`，避免营销外壳下正文远宽于控制台；控制台镜像
使用嵌入模式，不重复叠加文档组件的横向内边距，避免正文两侧留白过大。
