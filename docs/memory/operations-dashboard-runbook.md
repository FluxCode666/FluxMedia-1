...
创作留存和支付生命周期事实。生产发布在新 Web 事实写入路径启动后调用
`operations.ensureCurrentEpoch`：仅当 epoch 缺失时，按 `APP_TIME_ZONE` 当前自然日及其
UTC 零点初始化；已有不可变 epoch 时原样跳过。迁移本身仍不得写入 epoch。
...
正常生产发布不需要人工输入日期。部署流水线在新 Web 启动后、宣告发布成功前执行：
...
空表时 operation 使用生产 `APP_TIME_ZONE` 和服务端当前时钟派生自然日与 UTC 零点，
原子写入 epoch 和审计；已有值时返回 `initialized: false`，不会随后续发布日期漂移。
命令失败会阻断发布成功并停止新 Web，保持维护状态。调用方只提供发布身份，不能提供
日期、UTC 起点或 request ID；若 epoch 已被错误初始化，不得直接修改或删除，必须经过
新的受审批迁移修复。
...
SELECT admin_user_id, target_user_id, action, reason, metadata, created_at
...
`operations.initializeEpoch` 只用于识别永久修复上线前已经产生的历史审计；生产代码与
运维命令已删除该人工日期入口。
...
发布顺序固定为：并发预建运营复合索引、迁移结构、回填控制台统计读模型、启动新 Web
事实写入路径、自动确保当前应用日 epoch、通过健康检查、开启导出 worker、完成三类小范围
导出和浏览器冒烟、完成零差异对账、最后开放导航。
