...
import {
  configureFixtureStorage,
  createCompletedFixtureExportObject,
  createFixtureExports,
} from "./fixture-exports";
...
export async function completeLatestOperationsE2EExport(
  ...
  try {
    ...
    const epoch = await ensureFixtureEpoch(client);
    const object = await createCompletedFixtureExportObject(
      environment,
      task.id,
      epoch
    );
