...
-- fulfilled order、支付生命周期、成功产物与导出高水位的 ORDER BY/tuple seek
-- 完全一致。
...
CREATE INDEX IF NOT EXISTS "user_web_visit_created_cursor_idx"
  ON "user_web_visit" (
    "created_at" DESC,
    "user_id" DESC,
    "app_date" DESC
...
CREATE INDEX IF NOT EXISTS "user_output_usage_event_created_cursor_idx"
  ON "user_output_usage_event" (
    "created_at" DESC,
    "output_kind" DESC,
    "source_task_id" DESC
...
CREATE INDEX IF NOT EXISTS "payment_lifecycle_event_recorded_id_idx"
  ON "payment_lifecycle_event" ("recorded_at" DESC, "id" DESC);
--> statement-breakpoint

CREATE INDEX IF NOT EXISTS "credit_usage_projection_entry_projected_cursor_idx"
  ON "credit_usage_projection_entry" (
    "projected_at" DESC,
    "transaction_id" DESC
  );
