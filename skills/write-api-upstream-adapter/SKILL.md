...
- 异步图片只在当前进程轮询，进程崩溃后不能恢复远端任务或保证 exactly-once；
...
若图片生成响应可能返回异步任务，还要确认供应商是否支持幂等键、客户端重试是否会
重复计费。当前平台图片标准请求不向供应商转发统一幂等键；必须把进程重启后的远端
孤儿、重复生成和额外费用列为上线风险，不能把进程内轮询描述成持久恢复。
...
活跃生成请求，并监控 `api_upstream_image_task_orphan_risk`。
