...
function modelPath() {
    return (process.env.REALESR_MODEL_PATH?.trim() ||
...
async function runTile(session, hwc, w, h) {
    const area = w * h;
    ...
    if (!out)
        throw new Error("superResolve: 模型输出缺失");
    const dims = out.dims;
 ...
 * 副作用：CPU 密集；大图分块以限内存。
 */
export async function superResolve(image) {
