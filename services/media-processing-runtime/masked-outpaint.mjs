 ...
 */
export function planOutpaintTiles(targetW, targetH, tile = OUTPAINT_TILE, stepFraction = OUTPAINT_STEP_FRACTION) {
 ...
 */
export function tileKeepInset(plan, tile) {
...
async function buildTileMask(w, h, left, top) {
...
export const OUTPAINT_MAX_WORKING = 2 * OUTPAINT_TILE - Math.round(OUTPAINT_TILE * OUTPAINT_OVERLAP_FRACTION);
/**
 * 编排：掩码顺序外绘修复。
 *
 * @param image 输入图片字节
 ...
 */
export async function maskedOutpaintImage(image, targetLongEdge, editTile, superResolve) {
    const meta = await sharp(image).metadata();
    const sW = meta.width ?? 0;
    const sH = meta.height ?? 0;
    if (!sW || !sH)
        return { buffer: image, tilesRepaired: 0, tilesTotal: 0 };
    ...
    for (let i = 0; i < plan.tiles.length; i++) {
        ...
        try {
            ...
            const edited = await editTile(tilePng, mask, { col: t.col, row: t.row, cols: plan.cols, rows: plan.rows }, t.w, t.h, i);
    ...
    if (targetLongEdge > workLong) {
        ...
        buffer = await upscaleTo(buffer, Math.round(workW * scale), Math.round(workH * scale), superResolve);
    }
 ...
 */
async function upscaleTo(image, w, h, superResolve) {
    const meta = await sharp(image).metadata();
...
function extractTile(canvas, canvasW, t) {
 ...
 */
export function blackenNewRegion(tileRaw, t, left, top) {
 ...
 * 注：export 供单测。
 */
export function findBestOffset(canvas, canvasW, t, edited, left, top, range = OUTPAINT_ALIGN_RANGE, stride = OUTPAINT_ALIGN_STRIDE) {
    const { w, h } = t;
    let bestDx = 0;
    let bestDy = 0;
 ...
 */
export function slideAlignEditedTile(canvas, canvasW, t, edited, left, top) {
    const { w, h } = t;
