...
function modelPath() {
    return (process.env.SCUNET_MODEL_PATH?.trim() ||
...
export async function restoreImage(image) {
    try {
        ...
        const restored = await withRestoreLock(async () => {
            ...
            const paddedWidth = Math.ceil(W / 64) * 64;
            const paddedHeight = Math.ceil(H / 64) * 64;
            const src = await sharp(image).removeAlpha().extend({ top: 0, left: 0, right: paddedWidth - W, bottom: paddedHeight - H, extendWith: "mirror" }).raw().toBuffer();
            ...
            const result = await session.run({
                [inputName]: new ort.Tensor("float32", chw, [1, 3, paddedHeight, paddedWidth]),
            });
            const outT = result[outputName];
            if (!outT)
                throw new Error("restoreImage: 模型输出缺失");
            const od = outT.data;
            ...
            return sharp(buf, { raw: { width: paddedWidth, height: paddedHeight, channels: 3 } })
