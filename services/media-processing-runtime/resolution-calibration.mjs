 ...
 * 出图分辨率校准。
 *
 * 职责：部分上游会返回分辨率明显低于请求的图。当实际边长 < 目标边长的 2/3
 ...
 */
export function shouldSuperResolve(actual, target, ratio = SUPER_RESOLUTION_TRIGGER_RATIO) {
    if (!actual || !target)
        return false;
    const actualEdge = Math.max(actual.width, actual.height);
    const targetEdge = Math.max(target.width, target.height);
    if (actualEdge <= 0 || targetEdge <= 0)
        return false;
 ...
 */
export async function calibrateImageResolution(image, requestedSize) {
