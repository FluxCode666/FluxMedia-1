...
- **视频公开状态破坏性更新**：新旧视频创建、查询、站内页面与回调本次直接统一为 OpenAI 四态；旧 `pending`、`submitting`、`processing`、`needs_attention` 不再作为公开状态返回。接口地址及三个时长请求字段均未废弃。
