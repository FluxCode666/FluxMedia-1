...
- **OpenAI 风格视频地址与自动恢复**：新增并长期支持 `POST /v1/videos` 与 `/api/v1/videos`，旧 `*/videos/generations` 地址继续等价可用；新旧入口均在请求侧兼容 `seconds`、`duration`、`duration_seconds`，并统一公开 `queued`、`in_progress`、`completed`、`failed` 四态。API 供应商创建失败改为按账号快照有限重试、耗尽后自动切号，容量等待超时或所有账号耗尽后幂等退款；管理端全局使用记录可查看每次安全失败原因，并提供稳定结构化事件和告警标识文档。
