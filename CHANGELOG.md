...
- **OpenAI 风格视频地址与自动恢复**：新增并长期支持 `POST /v1/videos` 与 `/api/v1/videos`；旧 `*/videos/generations` 地址在下线前继续等价可用，但已在 API 文档标记为即将废弃下线，具体下线版本另行发布。新旧入口均在请求侧兼容 `seconds`、`duration`、`duration_seconds`，并统一公开 `queued`、`in_progress`、`completed`、`failed` 四态。API 供应商创建失败改为按账号快照有限重试、耗尽后自动切号，容量等待超时或所有账号耗尽后幂等退款；升级前显式 API `submit_uncertain` 完整快照自动迁移到重试，不完整快照只退款并输出一次专用告警；管理端全局使用记录可查看每次安全失败原因，并提供稳定结构化事件和告警标识文档。
...
- **视频公开状态破坏性更新**：新旧视频创建、查询、站内页面与回调本次直接统一为 OpenAI 四态；旧 `pending`、`submitting`、`processing`、`needs_attention` 不再作为公开状态返回。`duration`、`duration_seconds` 与 `seconds` 请求字段继续兼容；旧 `*/videos/generations` 地址进入文档废弃迁移期，但本版本尚未下线。
...
- **Nano Banana 2 模型 ID 统一**：将平台模型 ID 从 `nano-banana2` 更正为 `nano-banana-2`，并迁移已有配置与历史记录。
