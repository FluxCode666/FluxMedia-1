...
export const listTicketsAction = protectedAction
  ...
  .action(async ({ parsedInput }): Promise<TicketListOutput> => {
    ...
    const raw = await requestGoJson<{
      items: TicketListOutput["records"];
      page: number;
      pageSize: number;
      total: number;
      totalPages: number;
...
export const listTicketMessagesAction = protectedAction
  ...
  .action(async ({ parsedInput }): Promise<TicketMessageListOutput> => {
    ...
    const raw = await requestGoJson<{
      ticket: TicketMessageListOutput["ticket"];
      ticketUser?: TicketMessageListOutput["ticketUser"];
      messages: {
        items: TicketMessageListOutput["messages"]["records"];
        page?: number;
        pageSize?: number;
        total?: number;
        totalPages?: number;
    ...
    return {
      ...
      messages: {
        records: msg.items ?? [],
        page: msg.page ?? parsedInput.page,
        pageSize: msg.pageSize ?? parsedInput.pageSize,
        totalCount: msg.total ?? msg.items?.length ?? 0,
        totalPages: msg.totalPages ?? 1,
      },
    };
  });
...
export const markTicketSeenAction = protectedAction
  ...
  .action(async ({ parsedInput }): Promise<MarkTicketSeenOutput> => {
    return requestGoJson<MarkTicketSeenOutput>(
